// app.js
// Image Align Tool_50: Sorting + Stitch/Slice
// Base: Image Align Tool_41
// Changes in _50:
// - Fix Miro notifications API usage: showWarning now falls back to notifications.show()/showError().
// - Keep huge-mode init failures visible without crashing on missing showWarning.
// - Pair with a more robust worker that tries ImageDecoder first and falls back to createImageBitmap + OffscreenCanvas.
  ...
  try {
    ...
    if (kind === "showWarning") {
      if (typeof notifications.show === "function") {
        await notifications.show({ message: safeMessage, type: "error" });
        return;
      }
      if (typeof notifications.showError === "function") {
    ...
    if (typeof notifications.show === "function") {
      await notifications.show({ message: safeMessage, type: kind === "showError" ? "error" : "info" });
      return;
    }
  } catch (e) {
